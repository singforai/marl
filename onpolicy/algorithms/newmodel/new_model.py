...
class NewModel():
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        if self.use_joint_action_loss:
            ...
            old_action_log_probs_batch_copy = (
                old_action_log_probs_batch.reshape(
                    -1, self.num_agents, old_action_log_probs_batch.shape[-1]
                )
                .sum(dim=(1, -1), keepdim=True)
                .reshape(-1, 1)
            )
        ...
        if self._use_max_grad_norm:
            actor_grad_norm = nn.utils.clip_grad_norm_(self.policy.model.parameters(), self.max_grad_norm)
        else:
            actor_grad_norm = get_gard_norm(self.policy.model.parameters())
        ...
        if self._use_max_grad_norm:
            critic_grad_norm = nn.utils.clip_grad_norm_(self.policy.model.parameters(), self.max_grad_norm)
        else:
            critic_grad_norm = get_gard_norm(self.policy.model.parameters())
    ...
    def train(self, buffer, update_actor=True):
        ...
        for _ in range(self.ppo_epoch):
            data_generator = buffer.feed_forward_generator(advantages, self.num_mini_batch, self.data_chunk_length)
